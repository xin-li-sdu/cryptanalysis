...
int main(int argc, const char* argv[]) {
    ...
    int round = 3;  // 加密轮数
    int n = 10000; // 要生成的明文对数量
    int num = 0;//记录符合输出差分的数量
    uint64_t desiredXOR = 0x0020000800000400;//目标异或值
    uint64_t key = 0x0123456789ABCDEF;    // 主密钥

    std::cout << "aim: " << desiredXOR<<std::endl;
    std::vector<std::pair<uint64_t, uint64_t>> plaintextPairs;//存储明文对的容器
    
    while (plaintextPairs.size() < n) //寻找明文对
    {
        uint64_t x = generateRandomInput();
        uint64_t y = x ^ desiredXOR;
        if (x ^ y == desiredXOR) {
            plaintextPairs.push_back(std::make_pair(x, y));
        }
    }
    for (const auto& pair : plaintextPairs)
    {
        uint64_t result1 = des(pair.first, key, round, 'e'); //e:表示加密  d:表示解密
        uint64_t result2 = des(pair.second, key, round, 'e'); //e:表示加密  d:表示解密
        if ((result1 ^ result2) == desiredXOR)
        {
            num++;
            //std::cout << num << std::endl;
            std::cout << "plaintext pair: " << pair.first << " and " << pair.second << std::endl;
            std::cout << "p: " << (pair.first ^ pair.second);
            std::cout << std::endl;
            std::cout <<"c: " << (result1 ^ result2)<<std::endl;
    ...
    //uint64_t input = 0x0020000800000400;
    double ratio = static_cast<double>(num) / n * 100;
    std::cout << std::fixed << std::setprecision(2) << ratio << "%" << std::endl;// 设置输出为固定小数点后两位的浮点数，并加上百分号
