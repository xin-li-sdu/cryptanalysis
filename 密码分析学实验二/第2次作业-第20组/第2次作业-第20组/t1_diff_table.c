...
int dif_table[8][64][32];

uint8_t S_box(int v,int s_input){
    char row, column;
    row = (s_input & 1)|(s_input >> 4 & 2);
    column = (s_input >> 1 & 15);
    // printf("%d %d %d\n",s_input, row, column);
    return S[v][16*row + column];
}
...
int main(){
    memset(dif_table, 0, sizeof(dif_table));
    //首先构造差分分布表
    for(int v = 0; v < 8; v++){
        for(int d = 0; d < 64; d++){
            for(int i = 0; i < 64; i++){                
                dif_table[v][d][S_box(v, i)^S_box(v, i^d)]++;
            }
        }
        printf("dif_S%d\n",v);
        for(int i = 0; i < 64; i++){
            //输入差分只选择2，4和6，即000100，001000，001100，
            if(i!=4 & i!=8 & i!=12) continue;
            printf("input_diff:[%02d]\t",i);
            int maxn = 0, num = 0;
            for(int j = 0; j < 1<<4; j++){
                if(maxn < dif_table[v][i][j]){
                    maxn = dif_table[v][i][j];
                    num = j;
                }
            }
            printf("num:%d\t out_diff:%x\t", maxn, num);
            printf("\n");
        }
        printf("\n");
    }
